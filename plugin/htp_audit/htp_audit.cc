...
static int htp_audit_init_lock()
{
...
static void htp_audit_deinit_lock()
{
  if (!lock_initialized)
    return;
...
inline void htp_audit_lock_filter_and_var()
{
  mysql_mutex_lock(&LOCK_filter_and_var);
}

inline void htp_audit_unlock_filter_and_var()
{
...
static void htp_audit_init_filter_item(filter_item_t *item)
{
...
static void htp_audit_init_filter()
{
...
static void htp_audit_deinit_filter()
{
  filters.clear();
}
  ...
  , const char *sub_class, int sub_len, int *sub_class_int)
{
  if (strncasecmp(main_class, HTP_AUDIT_EVENT_GENERAL_CLASS, main_len) == 0)
  {
    *main_class_int = MYSQL_AUDIT_GENERAL_CLASS;
    ...
    /* 现在不支持，如果进行配置，则报告错误
    if (strncasecmp(sub_class, HTP_AUDIT_EVENT_GENERAL_SUB_LOG, sub_len) == 0)
    {
      return -1;
    }
    else if (strncasecmp(
...
static int htp_audit_parse_event(const char *event
                                   , int event_len, filter_item_t *item)
{
  ...
  while (index < event_len)
  {
    ...
    htp_audit_get_event_inchar(event_pos, single_len
                                , &main_class, &main_class_len
                                , &sub_class, &sub_class_len);

    r = htp_audit_get_event_init(main_class, main_class_len, &main_class_int
...
static int htp_audit_get_kv_unit(const char *current, const char **next
...
static int htp_audit_check_value_valid(const char *value, int length)
{
...
static int htp_audit_parse_kv_unit(const char *current, const char ** next
                                     , filter_item_t *item)
{
  const char *key = NULL;
  const char *value = NULL;
...
static int htp_audit_parse_filter(const char *filter_str, filter_item_t *item)
{
...
static int htp_audit_add_filter(filter_item_t *item)
{
  for (int i = 0; i < MAX_FILTER_ITEMS; i++)
  {
...
static int htp_audit_find_filter_by_name(const char *name)
{
...
static int htp_audit_remove_filter(remove_parse_t *removes)
{
  int i;
...
static
int
htp_audit_remove_rule_check_exist(remove_parse_t *removes)
{
  int i;
...
static int htp_audit_parse_remove_input(
...
htp_audit_filter_event(event_info_t *info, filter_item_t *item
...
inline filter_result_enum htp_audit_filter_event(event_info_t *info
                                                   , unsigned int event_class)
{
  if (filters.size() == 0)
    return NOT_AUDIT_EVENT;
...
*/
static struct st_mysql_show_var htp_audit_status[]=
{
  { "Htp_audit_called",
...
static void htp_audit_init_status()
{
  number_of_calls= 0;
...
static void htp_audit_deinit_status()
{
  //do nothing now
}
...
static char htp_audit_log_file[HTP_AUDIT_CONFIG_MAX_FILE_NAME + 1];
static char htp_audit_error_log_file[HTP_AUDIT_CONFIG_MAX_FILE_NAME + 1];
...
static struct st_mysql_sys_var *htp_audit_sys_var[] = {
...
static void htp_audit_init_variable()
{
...
static void htp_audit_deinit_variable()
{
...
static int htp_audit_reorg_filter_item(filter_item_t *filter_item)
{
  return 0;
}
...
static int
htp_audit_process_general_event(
...
static int
htp_audit_process_connection_event(
  MYSQL_THD thd __attribute__((unused))
  , unsigned int event_class
  ,const void *event)
{
...
static int
htp_audit_process_startup_event(
  MYSQL_THD thd __attribute__((unused))
  , unsigned int event_class
  ,const void *event)
{
...
static int
htp_audit_process_command_event(
  MYSQL_THD thd __attribute__((unused))
  , unsigned int event_class
  ,const void *event)
{
...
static int
htp_audit_process_table_access_event(
  MYSQL_THD thd __attribute__((unused))
  , unsigned int event_class
  ,const void *event)
{
...
static int
htp_audit_process_variable_event(
  MYSQL_THD thd __attribute__((unused))
  , unsigned int event_class
  ,const void *event)
{
...
static void htp_audit_process_event(MYSQL_THD thd __attribute__((unused)),
                           unsigned int event_class,
                           const void *event)
{
  switch (event_class)
